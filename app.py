...
def check(email):
    url = "https://u6ralygmpd.execute-api.ap-south-1.amazonaws.com/plasma/getdata?email="+email
...
def register():
    ...
    if('errorType' in check(x[1])):
        url = "https://u6ralygmpd.execute-api.ap-south-1.amazonaws.com/plasma/registration?"+params
...
@app.route('/stats')
def stats():
    url = " https://u6ralygmpd.execute-api.ap-south-1.amazonaws.com/plasma/getbloodgroupdata"
    response = requests.get(url)
    r = response.json()
    print(r)
...
def requested():
    ...
    #print(bloodgrp)
    url = "https://u6ralygmpd.execute-api.ap-south-1.amazonaws.com/plasma/requestonbloodgroup?blood="+bloodgrp
    status = requests.request("GET",url)
    a=status.json()
    emailids=[]
    for i in a:
